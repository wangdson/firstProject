...
public class TagShowActionLite extends ActionSupport{
	...
	public String execute() throws Exception {
		...
		//setTaglist(tagServ.getAllTagList());//填充表格	
		return "tag";
	}
	
	public String showTagBook() throws Exception {//填充类别和标签的下拉菜单
		forcateList(cateList,cateServ.getAllCateList(0));//填充类别下拉菜单
	...
	public String tagSelect() throws Exception {//选择标签后 获得booklist 填充表格
		if (getTagSel()!=null) {
...
/*		if(file.exists()){   
		...
		if (getTagsearch()!=null && !getTagsearch().equals("")) {
			...
			for (int i = 0; i < strlist.size(); i++) {//循环每个相关标签 ，获得书目LIST
				log.info("Integer.parseInt(strlist.get(i).toString())="+strlist.get(i).toString());
				List<PaperBag> templist=bookServ.getPaperBagByTagId(Integer.parseInt(strlist.get(i).toString()));//循环每个相关标签 ，获得小书目LIST
				//booklist.addAll(templist);
				log.info("templist.size()="+templist.size());
				for (int j = 0; j < templist.size(); j++) {//把每个小书目LIST中的对象放到大数目LIST中
...
/*		forcateList(cateList,cateServ.getAllCateList());//填充类别下拉菜单
		...
		if (getCateSel()!=null) {
			if (!getCateSel().toString().equals("--添加新类别--")) {
				String tagname="";
				Category cate=cateServ.searchCateByName(getCateSel());//根据类别名字获得类别
				taglist=tagServ.getTagByCategoryId(String.valueOf(cate.getCategoryId()));//根据类别ID 获得taglist
				for (int i = 0; i < taglist.size(); i++) {
